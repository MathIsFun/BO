...
//				}
					...
					for (int i = 0; i < amountNodes; i++) {
						for (int j = i + 1; j < amountNodes; j++) {
							if (Math.random() < probability) {
								g.addEdge(edgeFactory.create(), nodeList.get(i), nodeList.get(j));
								matrix.addPermission(i, j);
							}
						}
					 ...
					 frame = new JFrame();
					 CyclePane = new JPanel();
						CyclePane.setBorder(new EmptyBorder(5, 5, 5, 5));
						...
						CyclePane.add(panel_1, BorderLayout.CENTER);
		...
		btnUruchom.addActionListener(new ActionListener() {
	     ...
	     series[0] = new XYSeries("Best Adaptation");
	        ...
	        series[2] = new XYSeries("Expected Adaptation");
